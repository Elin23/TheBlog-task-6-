...
export default function SubscribeSection({ sectionName, title, paragraph, placeholder, Hint, link, linkLabel }: SectionProps) {
    return (
        <div className="flex flex-col items-start md:items-center dark:text-white font-inter" data-aos="fade">
            ...
            <p className="mb-10 lg:w-[750px] font-normal text-xl md:text-center lg:px-5 text-gray-500 dark:text-lightGray">{paragraph}</p>
            <form className="sm:w-auto w-full ">
                ...
                <p className="font-normal text-sm md:text-base text-gray-500 dark:text-lightGray  ">{Hint} <Link to={link} className="underline">{linkLabel}</Link></p>
            </form>
        </div>
    )
}
